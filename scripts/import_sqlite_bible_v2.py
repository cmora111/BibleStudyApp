...
def print_describe(conn: sqlite3.Connection) -> None:
...
def detect_source_schema(conn: sqlite3.Connection):
    ...
    for table in tables:
        ...
        for candidate in wanted_sets:
            if set(candidate).issubset(cols):
                return table, candidate[0], candidate[1], candidate[2], candidate[3]
    raise RuntimeError("Could not auto-detect a verse table. Run with --describe.")

def fetch_rows(conn: sqlite3.Connection, table: str, book_col: str, chapter_col: str, verse_col: str, text_col: str):
    query = (
        f"SELECT {qident(book_col)}, {qident(chapter_col)}, {qident(verse_col)}, {qident(text_col)} "
        f"FROM {qident(table)} ORDER BY {qident(book_col)}, {qident(chapter_col)}, {qident(verse_col)}"
    )
...
def main() -> int:
    ...
    if args.table and args.book_col and args.chapter_col and args.verse_col and args.text_col:
        table, book_col, chapter_col, verse_col, text_col = (
            args.table, args.book_col, args.chapter_col, args.verse_col, args.text_col
        )
    else:
