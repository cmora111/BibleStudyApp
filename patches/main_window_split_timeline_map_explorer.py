...
class UltimateBibleApp:
    ...
    def build_timeline_tab(self, parent: ttk.Frame) -> None:
        ...
        self.map_preview_status_var = tk.StringVar(value="Interactive map opens in your browser. Select a timeline event to generate a focused map.")
        ...
        ttk.Button(map_btns, text="Preview Selected Event Map", command=self.preview_selected_timeline_map).pack(side="left")
        ttk.Button(map_btns, text="Open Selected Event Map", command=self.open_selected_timeline_location_on_map).pack(side="left", padx=(6, 0))
        ttk.Button(map_btns, text="Open Full Timeline Map", command=self.open_timeline_map).pack(side="left", padx=(6, 0))
    ...
    def update_graph_view_for_event(self, event):
        ...
        lines.append("")
        lines.append(f"Reference: {event.reference}")
        lines.append(f"Time: {event.time_label or 'Unknown'}")
        lines.append(f"Location: {event.location_name or 'Unknown'}")
        lines.append("")
        lines.append("Nodes")
        for node in nodes:
    ...
    def update_map_explorer_for_event(self, event, *, auto_generate: bool = True):
        if hasattr(self, "map_focus_label_var"):
            self.map_focus_label_var.set(f"Map focus: {event.title}")
        if hasattr(self, "map_meta_output"):
            ...
            lines = [
                ...
                f"Time: {event.time_label or 'Unknown'}",
                f"Location: {event.location_name or 'Unknown'}",
                f"Coordinates: {event.latitude}, {event.longitude}" if event.latitude is not None and event.longitude is not None else "Coordinates: unavailable",
            ...
            lines.extend(["", event.summary or "(no summary)"])
            self.map_meta_output.insert("1.0", "\n".join(lines))

        if event.latitude is None or event.longitude is None:
            if hasattr(self, "map_preview_status_var"):
                self.map_preview_status_var.set("This event does not have coordinates, so a focused map cannot be generated.")
            return
        ...
        if auto_generate:
            try:
                ...
                self._last_highlighted_map = output
                if hasattr(self, "map_preview_status_var"):
                    self.map_preview_status_var.set(f"Focused map generated: {output}")
            except Exception as exc:
                if hasattr(self, "map_preview_status_var"):
                    self.map_preview_status_var.set(f"Map preview generation failed: {exc}")

    def preview_selected_timeline_map(self):
        selection = self.timeline_list.curselection()
        if not selection or not getattr(self, "_timeline_events_cache", None):
            return
        event = self._timeline_events_cache[selection[0]]
        self.update_map_explorer_for_event(event, auto_generate=True)
