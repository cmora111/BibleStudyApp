...
class UltimateBibleApp:
    ...
    def navigate_graph_node(self, node_type: str, label: str):
        ...
        if node_type == "theme":
            self.run_semantic_search_for_query(value)
            try:
    ...
    def _open_graph_node_menu(self, event, node_type: str, label: str):
        try:
            ...
            self._graph_nav_menu.add_command(
                ...
                command=lambda nt=node_type, lb=label: self.navigate_graph_node(nt, lb),
            )
    ...
    def update_graph_view_for_event(self, event):
        ...
        for idx, node in enumerate(nodes):
            ...
            self.graph_output.tag_bind(tag, "<Button-1>", lambda e, nt=node.node_type, lb=node.label: self.navigate_graph_node(nt, lb))
    ...
    def focus_on_event(self, event, *, open_compare_tab: bool = False, update_map: bool = False):
        ...
        if active_tab == "Knowledge Graph":
            self.root.after(150, lambda: self.update_graph_view_for_event(event))
        else:
            self.root.after(175, lambda: self.run_semantic_search_for_query(event.title))
